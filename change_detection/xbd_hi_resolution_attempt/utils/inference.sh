...
input=""
input_post=""
inference_base="/gws/nopw/j04/ai4er/users/jl2182/gtc-exposure/change_detection/xBD/xView2_baseline/utils/inference_base"
LOGFILE="/gws/nopw/j04/ai4er/users/jl2182/gtc-exposure/change_detection/xBD/xView2_baseline/utils/inference_logfile"
XBDIR=""
virtual_env=""
...
printf  "$input\n" 
printf  "$localization_weights\n"
printf  "$XBDIR\n"
printf  "$label_temp\n"
printf  "${input_image%.*}\n"
printf  "$LOGFILE\n"
echo $(which python)
python ./inference.py --input "$input" --weights "$localization_weights" --mean "$XBDIR"/weights/mean.npy --output "$label_temp"/"${input_image%.*}".json >> "$LOGFILE" 2>&1
printf "We've made it past here"
printf "\n" >> "$LOGFILE"
...
python ./process_data_inference.py --input_img "$disaster_post_file" --label_path "$label_temp"/"${input_image%.*}".json --output_dir "$inference_base"/output_polygons --output_csv "$inference_base"/output.csv >> "$LOGFILE" 2>&1

# Classifying extracted polygons 
python ./damage_inference.py --test_data "$inference_base"/output_polygons --test_csv "$inference_base"/output.csv --model_weights "$classification_weights" --output_json "$inference_base"/classification_inference.json >> "$LOGFILE" 2>&1
...
python "$XBDIR"/utils/combine_jsons.py --polys "$label_temp"/"${input_image%.*}".json --classes "$inference_base"/classification_inference.json --output "$inference_base/inference.json" >> "$LOGFILE" 2>&1
printf "\n" >> "$LOGFILE"

# Transforming the inference json file to the image required for scoring
printf "Finalizing output file" 
python "$XBDIR"/utils/inference_image_output.py --input "$inference_base"/inference.json --output "$inference_base/inference.png"  >> "$LOGFILE" 2>&1
