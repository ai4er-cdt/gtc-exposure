...
def label_informal_settlement(outpath_test):
    #dataframe to store tif information
    columns = ['Filename', 'Bands', 'Width', 'Height', 'Coordinates']
    dataframe = pd.DataFrame(columns = columns)


    for file in os.listdir(outpath_test):
        if file.endswith('.tif'):
            dataset = rasterio.open(outpath_test+ file)
    ...
    # for the drawn informal settlement polygons check to see if they intersect the sentinel tiles geometries
    count = 0
    interceptions = []
    for i in range(len(dataframe)):
        for polygon in informal_settlements():
            if (dataframe.loc[i]['Coordinates'].intersection(polygon).area/dataframe.loc[i]['Coordinates'].area)*100>=30:
                row = [i for i in dataframe.loc[i]]
                file = row[0]
                interceptions.append(file)
                print('Filename: {} \nBands: {} \nWidth: {} \nHeight: {} \nCoordinates: {}'.format(file,
                                                                                                   ...
                                                                                                   row[3],
                                                                                                   row[4]))
                os.system('mv {}{} {}{}'.format(outpath_test, file, outpath_test, 'inf_'+file))
